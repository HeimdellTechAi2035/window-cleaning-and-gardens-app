...
import {
  upsertAreaRound,
  assignPropertyToRound,
  findConflictingRound,
  deleteRoundIfEmptyAndAutoGenerated,
} from "@/lib/rounds";
...
export async function deleteCustomerAction(customerId: string): Promise<{ ok: true } | { error: string }> {
  try {
    const session = await requireSession();
    ...
    // Jobs/Transactions/Notifications all reference this customer (directly
    // or via a property) with RESTRICT foreign keys, so they have to be
    // cleared out before the customer (and its properties, via CASCADE)
    // can actually be deleted.
    const affectedRoundIds = await prisma.job.findMany({
    ...
    await prisma.$transaction([
      ...
      prisma.transaction.deleteMany({ where: { customerId: customer.id } }),
      prisma.job.deleteMany({ where: { property: { customerId: customer.id } } }),
      prisma.customer.delete({ where: { id: customer.id } }),
    ]);

    for (const { roundId } of affectedRoundIds) {
      await deleteRoundIfEmptyAndAutoGenerated(roundId);
    }
